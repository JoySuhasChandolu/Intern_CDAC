...
// 	inflight = bbr_quantization_budget(sk, 
// inflight);
// 	return inflight;
}
...
// target_cwnd = bbr_quantization_budget(sk, 
// target_cwnd);
...
// struct bbr *bbr = inet_csk_ca(sk);
// bool is_full_length =
// tcp_stamp_us_delta(tp->delivered_mstamp, 
// bbr->cycle_mstamp) >
// bbr->min_rtt_us;
// u32 inflight, bw;
...
// inflight = bbr_packets_in_net_at_edt(sk, 
// rs->prior_in_flight);
// bw = bbr_max_bw(sk);
...
// 	struct tcp_sock *tp = tcp_sk(sk);
// 	struct bbr *bbr = inet_csk_ca(sk);
// 	bbr->lt_last_stamp = div_u64(
// tp->delivered_mstamp, USEC_PER_MSEC);
...
// 	t = div_u64(
// tp->delivered_mstamp, USEC_PER_MSEC) - bbr->lt_last_stamp;
// 	if ((s32)t < 1)
 ...
 * trips.
 */
static void bbr_update_ack_aggregation(struct sock *sk,const struct rate_sample *rs)
{
...
class BBR:public CCC{
	...
	public:
		...
		virtual void init(){
			min_rtt = getPerfInfo()->msRTT;
			min_rtt_stamp = getPerfInfo()->msTimeStamp;
			probe_rtt_done_stamp = 0;
			probe_rtt_round_done = false;
			prior_cwnd = 0;
			idle_restart = false;
			extra_acked_interval_start = getPerfInfo()->msTimeStamp;
	...
	u64 extra_acked_interval_start;
